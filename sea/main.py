...
sio = socketio.Client()
SERVER_URL = ' https://unpractical-inadequate-nikola.ngrok-free.dev' # Твой адрес с Render
    ...
    while running:
        ...
        if mode == GameMode.ONLINE_MATCHMAKING:
            ...
            if online_state["last_shot_result"]:
                ...
                online_state["waiting"] = False # <-- СНИМАЕМ БЛОКИРОВКУ
        ...
        for event in pygame.event.get():
            ...
            if mode == GameMode.MENU:
                ...
                if event.type == pygame.MOUSEBUTTONDOWN and winner is None:
                    # Стреляем только если сейчас наш ход И мы не ждем ответа сервера
                    if online_state["my_turn"] and not online_state["waiting"]: # <-- ИЗМЕНЕНО
                        x, y = event.pos
                        if 450 <= x < 450 + GRID_SIZE * CELL_SIZE and 150 <= y < 150 + GRID_SIZE * CELL_SIZE:
                            ...
                            if p2_board.grid[gy][gx] == 0:
                                # Отправляем координаты выстрела на сервер и ставим блокировку!
                                online_state["waiting"] = True # <-- СТАВИМ БЛОКИРОВКУ
        ...
        if mode == GameMode.MENU:
            ...
            if winner:
                ...
                if online_state.get("waiting"):
                    status_text = "ОЖИДАНИЕ СЕРВЕРА..."
                    status_color = (255, 165, 0) # Оранжевый
                else:
